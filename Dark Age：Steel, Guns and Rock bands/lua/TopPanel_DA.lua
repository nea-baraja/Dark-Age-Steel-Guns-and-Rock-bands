...
function FaithConvertGreatPeoplePoints(playerID)
	...
	if PPK_FixBug[playerID] then whetherToConvert[playerID] = false; return; end
	...
	if faithBalance ~= nil then
		...
		if Amount1 > NeedToEndTheRound then Amount1 = NeedToEndTheRound end
		AddPROPHET = Amount1
...
function DA_ppkPlayerTurnActivated()
	...
	if not PPK_FixBug[playerID] then FaithConvertGreatPeoplePoints(playerID); end
end
...
function GetFaithTooltip()
	...
	if whetherToConvert[ePlayer] then szReturnValue = "[COLOR:ResMilitaryLabelCS]" .."本回合已转化" ..FaithAmount .." [ICON_FAITH] 信仰值为"..AddPROPHET .. " [ICON_GreatProphet] 大预言家的点数，在创建宗教后转化即会停止。[ENDCOLOR][NEWLINE]" ..szReturnValue end
...
function FormatValuePerTurn( value:number )
	...
	if whetherToConvert[ePlayer] then
		AddRed = AddRed + 1
		if AddRed == 3 then String1 = "[COLOR:ResMilitaryLabelCS]" ..String1 .. "[ENDCOLOR][ICON_GreatProphet]" end
	end
	return String1;
end
